...
def time_series_generate_hold(request):
    ...
    if request.method == 'GET':
        ...
        return JsonResponse({'value': [float(at)], 'time': [0], 'description' : "Generated by hold function with value "+str(at)})
    ...
    # Save a time series representing the hold function of the specified parameters
        ...
        data = {'name': request.POST.get('name', default='Unnamed'), 'value': [float(at)], 'time': [0], 'notes' : "Generated by hold function with value "+str(at)}
        serializer = TimeSeriesSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data, status=201)
...
def time_series_generate_ramp(request):
    if request.method == 'GET':
        ...
        return JsonResponse({'value': [float(start), float(end)], 'time': [0, int(duration)], 'description' : "Generated by ramp function with start value "+str(start)+", end value "+str(end)+", and duration "+str(duration)})
        ...
        data = {'name': request.POST.get('name', default='Unnamed'), 'value': [float(start), float(end)], 'time': [0, int(duration)], 'notes' : "Generated by ramp function with start value "+str(start)+", end value "+str(end)+", and duration "+str(duration)}
        serializer = TimeSeriesSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data, status=201)
...
def time_series_generate_sine(request):
    if request.method == 'GET':
        ...
        time = [frequency/10 * i for i in range(0, 21)]

        return JsonResponse({'value': value, 'time': time, 'description' : "Generated by sine function with frequency "+str(frequency)+", amplitude "+str(amplitude)+", offset_x "+str(offset_x)+", and offset_y "+str(offset_y)})
        ...
        data = {'name': request.POST.get('name', default='Unnamed'), 'value': value, 'time': time, 'notes' : "Generated by sine function with frequency "+str(frequency)+", amplitude "+str(amplitude)+", offset_x "+str(offset_x)+", and offset_y "+str(offset_y)}
        serializer = TimeSeriesSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data, status=201)
